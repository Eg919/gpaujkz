  ...
  <div class="gestion-historiques bg-gray-50/50 absolute top-14 bottom-0 left-0 right-0 overflow-hidden flex flex-col print:relative print:h-auto print:overflow-visible" :class="{ '': standalone }">
    ...
    <div id="pdf-export-zone" class="pdf-export-zone flex-1 flex flex-col min-h-0 mt-4 bg-white rounded-2xl shadow-xl border border-slate-200 mx-auto w-full max-w-[99%]" style="font-family: 'Outfit', sans-serif;">
      ...
      <div class="bg-white p-6 flex justify-between items-center border-b border-slate-100 text-slate-800">
        <div class="flex items-center gap-4">
          <div class="w-2 h-8 bg-indigo-600 rounded-full shadow-sm"></div>
          <div>
            <h2 class="text-xl font-bold tracking-tight text-slate-900 uppercase">Rapport d'Exécution : 2ème Trimestre</h2>
            <p class="text-slate-500 text-[10px] font-medium italic opacity-80">Phase de consolidation et suivi semestriel (T1-T2)</p>
          </div>
        </div>
        <div class="px-5 py-2.5 bg-slate-50 rounded-2xl border border-slate-100 flex flex-col items-end">
          ...
          <span class="text-indigo-600 font-extrabold text-xl leading-tight">{{ sessionsAnne }}</span>
        </div>
      </div>
      ...
      <div class="flex-1 overflow-auto scrollbar-thin print:overflow-visible">
        <table class="w-full border-collapse">
          <thead class="sticky top-0 z-30 bg-white shadow-sm border-b border-slate-200">
            <tr class="bg-slate-50 text-slate-700 font-black uppercase text-[10px]">
              <th rowspan="2" class="pt-4 pb-2 px-4 border-r border-slate-200 tracking-widest text-center align-top">N°</th>
              <th rowspan="2" class="pt-4 pb-2 px-4 border-r border-slate-200 tracking-widest text-left min-w-[300px] align-top">OBJECTIFS / ACTIVITÉS / TÂCHES</th>
              <th rowspan="2" class="pt-4 pb-2 px-4 border-r border-slate-200 tracking-widest text-center hidden md:table-cell align-top">CIBLE</th>
              <th rowspan="2" class="pt-4 pb-2 px-4 border-r border-slate-200 tracking-widest text-left align-top">DESCRIPTION DES TÂCHES</th>
              <th colspan="4" class="pt-4 pb-3 px-4 border-b border-r border-slate-200 tracking-widest text-center text-slate-400 font-black align-top">PROGRESSION TRIMESTRIELLE</th>
              <th rowspan="2" class="pt-4 pb-2 px-4 border-r border-slate-200 text-[10px] font-black uppercase tracking-widest text-center bg-blue-50/50 align-top">Avancement T2 (%)</th>
              <th rowspan="2" class="pt-4 pb-2 px-4 border-r border-slate-200 text-[10px] font-black uppercase tracking-widest text-center align-top">Budget T2 (FCFA)</th>
            ...
            <template v-for="(axe, indexAxe) in filteredAxes" :key="axe.id">
              ...
              <template v-for="(objectif, index) in axe.objectifs" :key="objectif.id">
                ...
                <tr class="bg-indigo-50/40 border-l-4 border-indigo-600 transition-all">
                  <td colspan="13" class="py-4 px-6 font-extrabold text-blue-900 border-b border-blue-100 text-sm uppercase">
                ...
                <template v-for="(effet, indexEffet) in objectif.effets" :key="effet.id">
                  ...
                  <tr class="bg-slate-50 border-l-4 border-slate-400/50">
                    <td colspan="13" class="py-3 px-10 font-bold text-slate-700 border-b border-slate-200 text-[11px] uppercase tracking-wide italic">
                  ...
                  <template v-for="(activite, indexActivite) in effet.activites" :key="activite.id">
                    ...
                    <tr class="hover:bg-slate-50 transition-colors group">
                      ...
                      <td class="py-3 px-4 border-r border-slate-100 text-center text-[13px] font-black text-indigo-700 bg-indigo-50/20 tabular-nums">
                        {{ activite.taux_t2 || 0 }}%
                      </td>
                      <td class="py-3 px-4 border-r border-slate-100 text-center text-[12px] font-black text-slate-900 tabular-nums">
                        {{ formatNumber(activite.coute_t2) }}
                      </td>
                  ...
                  <tr class="bg-blue-50/10">
                    ...
                    <td colspan="3" class="bg-slate-50/30"></td>
                  </tr>
                </template> <!-- Fin effet -->
                ...
                <tr class="bg-blue-600/5 border-b-2 border-blue-200">
                  ...
                  <td colspan="3" class="bg-blue-600/5"></td>
                </tr>
              ...
              <tr class="bg-slate-100 border-b-2 border-slate-300">
                ...
                <td colspan="3" class="bg-slate-100"></td>
              </tr>
            </template>
            
            <!-- Récapitulatif Final T2 -->
            <tr class="bg-white border-t-2 border-indigo-600">
                <td colspan="13" class="py-10 px-10">
                    <div class="flex justify-between items-center max-w-4xl ml-auto">
                        <div class="flex items-center gap-8 group">
                            <div class="text-right whitespace-nowrap">
                                <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1 group-hover:text-amber-500 transition-colors">Dépenses T2</span>
                                <p class="text-slate-400 text-[9px] italic font-medium leading-none">Total décaissé pour le trimestre</p>
                            </div>
                            <div class="h-10 w-px bg-slate-100 transition-all group-hover:h-12 group-hover:bg-amber-100"></div>
                        ...
                        <div class="flex items-center gap-8 group">
                            ...
                            <div class="px-6 py-3 rounded-2xl text-2xl font-black bg-indigo-900 text-white shadow-xl transition-all scale-100 hover:scale-105">
...
export default {
  ...
  methods: {
    ...
    calculerTotalEffet(effet) {
      ...
      return effet.activites.reduce((sum, act) => sum + Number(act.coute_t2 || 0), 0);
    },
    calculerMoyenneEffet(effet) {
      if (!effet.activites || effet.activites.length === 0) return 0;
      const sum = effet.activites.reduce((acc, act) => acc + (act.taux_t2 || 0), 0);
    ...
    calculerMoyenneObjectif(objectif) {
      ...
      const sum = objectif.effets.reduce((acc, eff) => {
        return acc + eff.activites.reduce((s, act) => s + (act.taux_t2 || 0), 0);
      }, 0);
    ...
    calculerMoyenneAxe(axe) {
      ...
      const sum = axe.objectifs.reduce((acc, obj) => {
        return acc + obj.effets.reduce((s, eff) => {
          return s + eff.activites.reduce((ss, act) => ss + (act.taux_t2 || 0), 0);
        }, 0);
      }, 0);
      return Math.round(sum / count);
    },
    ...
    exportToExcel() {
      try {
        const data = [
            [`RAPPORT D'EXÉCUTION DU 2ème TRIMESTRE - ${this.sessionsAnne}`],
            [],
            ["N°", "Activité", "Taux d'avancement T2 (%)", "Budget Décaissé T2 (FCFA)", "Partenaires", "Structure"]
        ];
        ...
        this.filteredAxes.forEach((axe, axeIdx) => {
            ...
            axe.objectifs.forEach((obj, idx) => {
                ...
                obj.effets.forEach((eff, effIdx) => {
                    ...
                    data.push(["", "Sous-Total Effet (T2)", `${this.calculerMoyenneEffet(eff)}%`, this.calculerTotalEffet(eff), ""]);
                });
                data.push(["", "Total Objectif Global (T2)", `${this.calculerMoyenneObjectif(obj)}%`, this.calculerTotalObjectif(obj), ""]);
            });
        });

        const ws = XLSX.utils.aoa_to_sheet(data);
        const wb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, ws, "Rapport T2");
        XLSX.writeFile(wb, `Rapport_T2_${this.sessionsAnne}.xlsx`);
      } catch (error) {
