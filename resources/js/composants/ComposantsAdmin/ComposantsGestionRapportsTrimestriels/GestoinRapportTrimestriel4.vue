    ...
    <div id="pdf-export-zone" class="pdf-export-zone flex-1 flex flex-col min-h-0 mt-4 bg-white rounded-2xl shadow-xl border border-slate-200 mx-auto w-full max-w-[99%]" style="font-family: 'Outfit', sans-serif;">
      ...
      <div class="bg-white p-6 flex justify-between items-center border-b border-slate-100 text-slate-800">
        <div class="flex items-center gap-4">
          ...
          <div>
            <h2 class="text-xl font-bold tracking-tight text-slate-900 uppercase">Bilan Annuel : 4ème Trimestre</h2>
            <p class="text-slate-500 text-[10px] font-medium italic opacity-80">Synthèse finale et clôture de l'exercice budgétaire (T1-T4)</p>
          </div>
        </div>
        <div class="px-5 py-2.5 bg-slate-50 rounded-2xl border border-slate-100 flex flex-col items-end">
          <span class="text-slate-400 text-[9px] uppercase tracking-widest block font-bold mb-0.5">Année de Référence</span>
        ...
        <table class="w-full border-collapse">
          <thead class="sticky top-0 z-30 bg-white shadow-sm border-b border-slate-200">
            <tr class="bg-slate-50 text-slate-700 font-black uppercase text-[10px]">
              ...
              <th colspan="4" class="pt-4 pb-3 px-4 border-b border-r border-slate-200 text-[10px] font-black uppercase tracking-widest text-center text-slate-400 font-black align-top">PROGRESSION TRIMESTRIELLE</th>
              <th rowspan="2" class="pt-4 pb-2 px-4 border-r border-slate-200 text-[10px] font-black uppercase tracking-widest text-center bg-blue-50/50 align-top">Avancement T4 (%)</th>
              <th rowspan="2" class="pt-4 pb-2 px-4 border-r border-slate-200 text-[10px] font-black uppercase tracking-widest text-center align-top">Budget T4 (FCFA)</th>
          ...
          <tbody class="divide-y divide-slate-100 text-slate-700">
            <template v-for="(axe, indexAxe) in filteredAxes" :key="axe.id">
              ...
              <tr class="bg-slate-100 border-l-4 border-slate-500 transition-all">
                <td colspan="13" class="py-4 px-6 font-extrabold text-slate-800 border-b border-slate-200 text-[14px] uppercase tracking-widest">
                  <div class="flex items-center gap-3">
                  ...
                  <template v-for="(activite, indexActivite) in effet.activites" :key="activite.id">
                    ...
                    <tr class="hover:bg-slate-50 transition-colors group">
                      ...
                      <td class="py-3 px-4 border-r border-slate-100 text-center text-[12px] font-black text-slate-900 tabular-nums">
                        {{ formatNumber(activite.coute_t4) }}
                      </td>
                    ...
                    <div class="flex justify-between items-center max-w-4xl ml-auto">
                        <div class="flex items-center gap-8 group">
                            <div class="text-right whitespace-nowrap">
                                <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1 group-hover:text-amber-500 transition-colors">Dépenses T4</span>
...
export default {
  ...
  methods: {
    ...
    calculerTotalEffet(effet) {
      ...
      return effet.activites.reduce((sum, act) => sum + Number(act.coute_t4 || 0), 0);
    },
    calculerMoyenneEffet(effet) {
      if (!effet.activites || effet.activites.length === 0) return 0;
      const sum = effet.activites.reduce((acc, act) => acc + (act.taux_t4 || 0), 0);
    ...
    calculerMoyenneObjectif(objectif) {
      ...
      const sum = objectif.effets.reduce((acc, eff) => {
        return acc + eff.activites.reduce((s, act) => s + (act.taux_t4 || 0), 0);
      }, 0);
    ...
    calculerMoyenneAxe(axe) {
      ...
      const sum = axe.objectifs.reduce((acc, obj) => {
        return acc + obj.effets.reduce((s, eff) => {
          return s + eff.activites.reduce((ss, act) => ss + (act.taux_t4 || 0), 0);
        }, 0);
      }, 0);
      return Math.round(sum / count);
    },
    ...
    exportToExcel() {
      try {
        const data = [
            [`RAPPORT D'EXÉCUTION DU 4ème TRIMESTRE - ${this.sessionsAnne}`],
            [],
            ["N°", "Activité", "Taux d'avancement T4 (%)", "Budget Décaissé T4 (FCFA)", "Partenaires", "Structure"]
        ];
        ...
        this.filteredAxes.forEach((axe, axeIdx) => {
            ...
            axe.objectifs.forEach((obj, idx) => {
                ...
                obj.effets.forEach((eff, effIdx) => {
                    ...
                    data.push(["", "Sous-Total Effet (T4)", `${this.calculerMoyenneEffet(eff)}%`, this.calculerTotalEffet(eff), ""]);
                });
                data.push(["", "Total Objectif Global (T4)", `${this.calculerMoyenneObjectif(obj)}%`, this.calculerTotalObjectif(obj), ""]);
            });
        });

        const ws = XLSX.utils.aoa_to_sheet(data);
        const wb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, ws, "Rapport T4");
        XLSX.writeFile(wb, `Rapport_T4_${this.sessionsAnne}.xlsx`);
      } catch (error) {
