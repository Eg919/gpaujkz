    ...
    <div id="pdf-export-zone" class="pdf-export-zone flex-1 flex flex-col min-h-0 mt-4 bg-white rounded-2xl shadow-xl border border-slate-200 mx-auto w-full max-w-[99%]" style="font-family: 'Outfit', sans-serif;">
      ...
      <div class="bg-white p-6 flex justify-between items-center border-b border-slate-100 text-slate-800">
        <div class="flex items-center gap-4">
          ...
          <div>
            <h2 class="text-xl font-bold tracking-tight text-slate-900 uppercase">Rapport d'Exécution : 3ème Trimestre</h2>
            <p class="text-slate-500 text-[10px] font-medium italic opacity-80">Suivi des réalisations et projections de fin d'année (T1-T3)</p>
          </div>
        </div>
        ...
        <table class="w-full border-collapse">
          <thead class="sticky top-0 z-30 bg-white shadow-sm border-b border-slate-200">
            <tr class="bg-slate-50 text-slate-700 font-black uppercase text-[10px]">
              ...
              <th colspan="4" class="pt-4 pb-3 px-4 border-b border-r border-slate-200 text-[10px] font-black uppercase tracking-widest text-center text-blue-500 align-top">PROGRESSION TRIMESTRIELLE</th>
              <th rowspan="2" class="pt-4 pb-2 px-4 border-r border-slate-200 text-[10px] font-black uppercase tracking-widest text-center bg-blue-50/50 align-top">Avancement T3 (%)</th>
              <th rowspan="2" class="pt-4 pb-2 px-4 border-r border-slate-200 text-[10px] font-black uppercase tracking-widest text-center align-top">Budget T3 (FCFA)</th>
                  ...
                  <template v-for="(activite, indexActivite) in effet.activites" :key="activite.id">
                    ...
                    <tr class="hover:bg-slate-50 transition-colors group">
                      ...
                      <td class="py-3 px-4 border-r border-slate-100 text-center text-[12px] font-black text-slate-900 tabular-nums">
                        {{ formatNumber(activite.coute_t3) }}
                      </td>
                    ...
                    <div class="flex justify-between items-center max-w-4xl ml-auto">
                        <div class="flex items-center gap-8 group">
                            <div class="text-right whitespace-nowrap">
                                <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1 group-hover:text-amber-500 transition-colors">Dépenses T3</span>
...
export default {
  ...
  methods: {
    ...
    calculerTotalEffet(effet) {
      ...
      return effet.activites.reduce((sum, act) => sum + Number(act.coute_t3 || 0), 0);
    },
    calculerMoyenneEffet(effet) {
      if (!effet.activites || effet.activites.length === 0) return 0;
      const sum = effet.activites.reduce((acc, act) => acc + (act.taux_t3 || 0), 0);
    ...
    calculerMoyenneObjectif(objectif) {
      ...
      const sum = objectif.effets.reduce((acc, eff) => {
        return acc + eff.activites.reduce((s, act) => s + (act.taux_t3 || 0), 0);
      }, 0);
    ...
    calculerMoyenneAxe(axe) {
      ...
      const sum = axe.objectifs.reduce((acc, obj) => {
        return acc + obj.effets.reduce((s, eff) => {
          return s + eff.activites.reduce((ss, act) => ss + (act.taux_t3 || 0), 0);
        }, 0);
      }, 0);
      return Math.round(sum / count);
    },
    ...
    exportToExcel() {
      try {
        const data = [
            [`RAPPORT D'EXÉCUTION DU 3ème TRIMESTRE - ${this.sessionsAnne}`],
            [],
            ["N°", "Activité", "Taux d'avancement T3 (%)", "Budget Décaissé T3 (FCFA)", "Partenaires", "Structure"]
        ];
        ...
        this.filteredAxes.forEach((axe, axeIdx) => {
            ...
            axe.objectifs.forEach((obj, idx) => {
                ...
                obj.effets.forEach((eff, effIdx) => {
                    ...
                    data.push(["", "Sous-Total Effet (T3)", `${this.calculerMoyenneEffet(eff)}%`, this.calculerTotalEffet(eff), ""]);
                });
                data.push(["", "Total Objectif Global (T3)", `${this.calculerMoyenneObjectif(obj)}%`, this.calculerTotalObjectif(obj), ""]);
            });
        });

        const ws = XLSX.utils.aoa_to_sheet(data);
        const wb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, ws, "Rapport T3");
        XLSX.writeFile(wb, `Rapport_T3_${this.sessionsAnne}.xlsx`);
      } catch (error) {
