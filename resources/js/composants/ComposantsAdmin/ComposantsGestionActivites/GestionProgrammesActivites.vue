  ...
  <div class="flex flex-col items-center min-h-screen" :class="{ '': standalone }">
    ...
    <div v-if="standalone" class="w-full bg-gray-50 shadow-md border-b border-gray-200 py-3 px-4 md:px-8 flex items-center mb-8">
      ...
      <div class="w-2/4 text-center">
        <h1 class="text-xl md:text-2xl font-black text-amber-500 uppercase tracking-tighter">Programmes d'Activités</h1>
      </div>
      <div class="w-1/4"></div>
    </div>
      <!-- Contenu principal -->
      <div class="w-full mt-4 px-4 md:px-6">
        ...
        <div v-if="!isMobile" class="flex w-full h-[calc(100vh-180px)] flex-col md:flex-row bg-white rounded-t-3xl shadow-2xl overflow-hidden border-t border-x border-gray-100">
          ...
          <div class="w-full md:w-2/5 overflow-y-auto px-6 py-6 h-full border-r border-gray-100 bg-gray-50/30">
            ...
            <p v-else class="text-gray-400 mt-10 text-center italic text-sm">Aucune activité trouvée.</p>
          </div>
          ...
          <div class="w-full md:w-3/5 flex flex-col mt-4 md:mt-0 h-full overflow-y-auto bg-white p-6">
            ...
            <div v-if="activiteIdSelectionne" class="flex justify-between items-center space-x-3 mb-6 p-4 bg-gray-50 rounded-2xl border border-gray-100 shadow-sm">
               <div class="flex flex-col">
                  <span class="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Etat de la session :</span>
                  <div class="flex items-center gap-2 mt-1">
                    <span 
                      class="w-2 h-2 rounded-full"
                      :class="activites.find(a => a.id === activiteIdSelectionne).etat_session === 'Clôturé' ? 'bg-rose-500' : 'bg-emerald-500'"
                    ></span>
                    <span class="text-xs font-bold text-gray-700 uppercase tracking-tighter">
                      {{ activites.find(a => a.id === activiteIdSelectionne).etat_session.replace('_', ' ') }}
                    </span>
                  </div>
               </div>
               <button 
                ...
                <button 
                    ...
                    class="bg-green-500 text-white px-4 py-2 rounded-xl shadow-sm transition-all duration-200 hover:bg-green-600 flex items-center text-sm font-bold relative group"
                    :class="{'opacity-40 cursor-not-allowed': !pouvonsModifier}"
                    :disabled="!pouvonsModifier"
                >
                    <i class="fas fa-edit mr-2"></i> 
                    <span>Modifier</span>
                    <div v-if="!pouvonsModifier && activites.find(a => a.id === activiteIdSelectionne).etat_session === 'Clôturé'" class="absolute -top-10 left-1/2 -translate-x-1/2 hidden group-hover:block bg-slate-800 text-white text-[9px] px-3 py-1.5 rounded-lg whitespace-nowrap shadow-xl">
            ...
            <div v-else class="flex flex-col items-center justify-center h-full text-gray-300">
                ...
                <span class="text-lg font-bold uppercase tracking-widest text-gray-400 px-10 text-center">Sélectionnez une activité du programme pour en consulter les détails</span>
            </div>
          </div>
        </div>
      </div>
